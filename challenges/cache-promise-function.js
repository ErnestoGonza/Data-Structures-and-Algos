...
const cachePromiseFunction2 = (get) => {
  ...
  return function (url) {
    if (cache.has(url)) return Promise.resolve(cache.get(url));
    else if (inProgressPromises.has(url)) return inProgressPromises.get(url);
    else {
      const promise = get(url).then((response) => {
        cache.set(url, response);
        inProgressPromises.delete(url);
        return response;
      });
      inProgressPromises.set(url, promise);
      return promise;
    }
  };
};
