...
const mergeRanges = intervals => {
  ...
  intervals.forEach((currentInt) => {
    const [start, end] = currentInt;
    const recent = results[results.length - 1];

    if(start <= recent[1]) recent[1] = Math.max(recent[1], end);

    else results.push(currentInt);
  });
  return results;
  ...
  //   if(previous[1] >= sorted[i][0]) {
  //       previous = [previous[0], Math.max(previous[1], sorted[i][1])];
  //   } else {
