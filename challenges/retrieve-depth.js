...
const retrieveDepth = (arr, depth, start = 1) => {
  
  if(start < depth) return retrieveDepth(arr.flat(), depth, ++start)
...
const flattenDepth = (arr, depth, start = 0) => {
  if(start < depth) return flattenDepth(arr.flat(), depth, ++start);
  return arr;
};
